...
#define TFT_CS                  (*((volatile uint8_t *) 0x40004C42))  /* Port 9 Output, bit 4 is TFT CS */
#define TFT_CS_BIT              0x01       // CS normally controlled by hardware
#define DC                      (*((volatile uint8_t *) 0x40004C42))  /* Port 9 Output, bit 2 is DC */
#define DC_BIT                  0x04
#define RESET                   (*((volatile uint8_t *) 0x40004C22))  /* Port 9 Output, bit 3 is RESET*/
...
uint8_t static writecommand(uint8_t c) {
  while((EUSCI_B0->IFG&0x0002)==0x0000){};    // wait until EUSCI_B0->TXBUF empty
  DC &= ~DC_BIT;
  EUSCI_B0->TXBUF = c;                        // command out
  while((EUSCI_B0->IFG&0x0001)==0x0000){};    // wait until EUSCI_B0->RXBUF full
  return EUSCI_B0->RXBUF;                     // return the response
}
...
void static commonInit(const uint8_t *cmdList) {
 ...
 // P9->DIR |= 0x1C;                        // make P9.2 (D/C), P9.3 (Reset), and P9.4 (TFT_CS) out
  ...
  EUSCI_B0->BRW = 3;
  // modulation is not used in SPI mode, so clear EUSCI_B0->MCTLW
 // EUSCI_B0->MCTLW = 0;
  //P9->SEL0 |= 0xB0;
